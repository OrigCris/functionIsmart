...
SELECT 
    ...
    c.data_inicio_curso,
    CASE 
        WHEN c.data_inicio_curso IS NULL THEN 'VETERANO'
        WHEN LEFT(CONVERT(VARCHAR(10), c.data_inicio_curso), 4) = '2025' THEN 'CALOURO'
        ELSE 'VETERANO'
    END AS flag_calouro_veterano
FROM tb_dm_universitarios AS u
LEFT JOIN data_facts_es_informacoes_curso AS c
    ON u.ra = c.ra
